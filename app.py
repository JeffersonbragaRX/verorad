...
with col_c:
    ...
    if st.session_state.modo_crop and st.session_state.img_raw:
        st.markdown('<div class="vr-crop-hint">✂️ Defina a região de recorte usando os controles abaixo. Ajuste até enquadrar apenas a mão e o punho.</div>', unsafe_allow_html=True)
        ...
        with col_x1:
            x1_pct = st.slider("Esquerda (%)", 0, 49, 0)
            y1_pct = st.slider("Superior (%)", 0, 49, 0)
        with col_x2:
            x2_pct = st.slider("Direita (%)", 51, 100, 100)
            y2_pct = st.slider("Inferior (%)", 51, 100, 100)
        ...
        draw.rectangle([x1, y1, x2, y2], outline="#2563EB", width=max(3, W//150))
        st.image(preview, use_container_width=True, caption="Prévia do recorte")
