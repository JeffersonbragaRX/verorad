...
# --- FUNÇÕES CORE (A sua lógica original mantida e otimizada) ---
...
with col_cfg:
    ...
    paste_result = paste_image_button(label="📋 Colar do Clipboard (Win+Shift+S)")
    upload = st.file_uploader("Ou arquivo", type=["png","jpg","jpeg"], label_visibility="collapsed")
    
    nova_img = None
    if paste_result and paste_result.image_data: nova_img = paste_result.image_data.convert("RGB")
    elif upload: nova_img = Image.open(upload).convert("RGB")
...
with col_img:
    ...
    if st.session_state.img_raw:
        st.image(st.session_state.img_raw, use_container_width=True, caption="Imagem pronta para análise")
